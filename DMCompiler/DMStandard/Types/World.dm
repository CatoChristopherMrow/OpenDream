/world
	...
	var/movement_mode = LEGACY_MOVEMENT_MODE as opendream_compiletimereadonly
	...
	var/url
	var/visibility = 0 as num //used to control server appearing on the hub - this will have to use ServerStatusCode.Offline/Online
	var/status = "" as text //used to display a message appearing under the server on the hub - this will have to use the server desc on the hub
	var/process
